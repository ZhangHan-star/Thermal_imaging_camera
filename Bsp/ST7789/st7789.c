 ...
 */
 #define HOR_LEN 	20	//	Alse mind the resolution of your screen!
...
// ST7789_WriteSmallData(0x02);
// ST7789_WriteSmallData(0x14);
    ...
    ST7789_WriteSmallData(0x35);			//	0.725v (default 0.75v for 0x20)
    ST7789_WriteCommand(0xC0);				//	LCMCTRL
    ST7789_WriteSmallData (0x2C);			//	Default value
    ST7789_WriteCommand (0xC2);				//	VDV and VRH command Enable
    ST7789_WriteSmallData (0x01);			//	Default value
    ST7789_WriteCommand (0xC3);				//	VRH set
    ST7789_WriteSmallData (0x07);			//	+-4.45v (defalut +-4.1v for 0x0B)
...
void ST7789_DrawPixel_LVGL(uint16_t x, uint16_t y, uint16_t color)
{
	if ((x < 0) || (x >= ST7789_WIDTH) ||
		 (y < 0) || (y >= ST7789_HEIGHT))	return;
 ...
 * @param bgcolor 背景颜色
 */
void ST7789_WriteOneNum(uint16_t x, uint16_t y, uint16_t Num, FontDef font, uint16_t color, uint16_t bgcolor)
{
    Num = Num % 10;
    ST7789_WriteChar(x, y, Num + 0x30, font, color, bgcolor);
}
 ...
 * @param bgcolor 背景颜色
 */
void ST7789_WriteNum(uint16_t x, uint16_t y, uint64_t Num, FontDef font, uint16_t color, uint16_t bgcolor)
{
    uint64_t temp_num = 0;
		uint64_t temp=0;
		uint8_t Weishu=0;
    ...
    do {
		...
		for (uint8_t i = 0; i < Weishu; i++)
		{
			...
			temp%=10;
			ST7789_WriteChar(x+(font.width*(Weishu-i)),y,temp+0x30,font,color,bgcolor);
