...
W25QXX_result_t w25qxx_init(W25QXX_HandleTypeDef *w25qxx, SPI_HandleTypeDef *hspi, GPIO_TypeDef *cs_port, uint16_t cs_pin) {
    ...
    if (id) {
        ...
        W25_DBG("Unknown device,ID: 0x%02x",id);
        result = W25QXX_Err;
    }
...
// }
W25QXX_result_t w25qxx_write(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint8_t *buf, uint32_t len) {
    const uint32_t sector_size = 4096; // 根据芯片调整扇区大小
    static uint8_t sector_buffer[4096];
    uint32_t start_sector = address / sector_size;
    uint32_t end_sector = (address + len - 1) / sector_size;

    W25_DBG("w25qxx_write with erase - address 0x%08lx len 0x%04lx", address, len);

    for (uint32_t sector = start_sector; sector <= end_sector; sector++) {
        uint32_t sector_addr = sector * sector_size;
        uint32_t offset_in_sector = address - sector_addr;
        uint32_t bytes_in_sector = sector_size - offset_in_sector;
        uint32_t write_len = (len < bytes_in_sector) ? len : bytes_in_sector;

        // 读取整个扇区到缓冲区
        if (w25qxx_read(w25qxx, sector_addr, sector_buffer, sector_size) != W25QXX_Ok) {
            return W25QXX_Err;
        }
        ...
        for (uint32_t i = 0; i < write_len; i++) {
            if (sector_buffer[offset_in_sector + i] != 0xFF) {
                need_erase = 1;
                break;
            }
        }
        ...
        if (need_erase) {
            // 合并用户数据到缓冲区
            memcpy(sector_buffer + offset_in_sector, buf, write_len);

            // 擦除扇区
            if (w25qxx_erase_sector(w25qxx, sector_addr) != W25QXX_Ok) {
                return W25QXX_Err;
            }

            // 写入整个扇区（使用原分页写入函数）
            uint32_t page_size = w25qxx->page_size;
            uint32_t pages = sector_size / page_size;
            for (uint32_t i = 0; i < pages; i++) {
                uint32_t page_addr = sector_addr + i * page_size;
                if (w25qxx_write_page(w25qxx, page_addr, sector_buffer + i * page_size, page_size) != W25QXX_Ok) {
                    return W25QXX_Err;
                }
            }
        } else {
            // 直接写入数据（使用原分页写入函数）
            if (w25qxx_write_page(w25qxx, address, buf, write_len) != W25QXX_Ok) {
                return W25QXX_Err;
            }
        }
...
/* 原写入函数重命名为w25qxx_write_page */
W25QXX_result_t w25qxx_write_page(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint8_t *buf, uint32_t len) {
    // 原函数的分页写入逻辑
    uint32_t first_page = address / w25qxx->page_size;
    uint32_t last_page = (address + len - 1) / w25qxx->page_size;
    ...
    for (uint32_t page = first_page; page <= last_page; ++page) {
        ...
        if (w25qxx_wait_for_ready(w25qxx, HAL_MAX_DELAY) != W25QXX_Ok) return W25QXX_Err;
        if (w25qxx_write_enable(w25qxx) != W25QXX_Ok) return W25QXX_Err;
        ...
        cs_on(w25qxx);
        if (w25qxx_transmit(w25qxx, tx, sizeof(tx)) == W25QXX_Ok) {
            if (w25qxx_transmit(w25qxx, buf + buffer_offset, write_len) != W25QXX_Ok) {
                cs_off(w25qxx);
                return W25QXX_Err;
            }
...
/* 新增擦除函数 */
W25QXX_result_t w25qxx_erase_sector(W25QXX_HandleTypeDef *w25qxx, uint32_t sector_addr) {
    if (w25qxx_wait_for_ready(w25qxx, HAL_MAX_DELAY) != W25QXX_Ok) return W25QXX_Err;
    if (w25qxx_write_enable(w25qxx) != W25QXX_Ok) return W25QXX_Err;
    ...
    cs_on(w25qxx);
    W25QXX_result_t res = w25qxx_transmit(w25qxx, tx, sizeof(tx));
    cs_off(w25qxx);
